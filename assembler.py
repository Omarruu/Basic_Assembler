class Assembler(object):
       ...
       """
        ...
        for line in self.__asm:
            if self.__islabel(line[0]):
                self.__address_symbol_table[line[0]]=LC
            else:
                if line[0]=='org':
                    LC=int(line[1],16)
                elif line[0]=='end':
    ...
    def __second_pass(self) -> None:
        ...
        LC=0
        for line in self.__asm:
            if (line[0]=='org' or 'end') or (self.__islabel(line[0]) and line[1]=='hex' or 'dec'):
                if line[0]=='org':
                    LC=int(line[1],16)
                else:
                    if line[0]=='end':
                        ...
                        if line[1]=='dec':
                            key=self.__format2bin(LC , 'dec' , 12)
                            value=self.__format2bin(line[2], 'dec' , 16)
                            self.__bin[key]=value
                            LC+=1
                        else:
                            key=self.__format2bin(LC , 'dec' , 12)
                            value=self.__format2bin(line[2], 'hex' , 16)
                            self.__bin[key]=value
                            LC+=1
            else:
                if line[0] in self.__mri_table:
                        if line[2] =='i':
                            ...
                            i=0
                        key=self.__format2bin(LC, 'dec' , 12)
                        value=i+self.mri_table[line[0]]+self.__address_symbol_table[line[1]]
                        self.__bin[key]=value
                        LC+=1
                else:
                    if line[0] in self.__rri_table or line[1] in self.__rri_table or line[0] in self.__ioi_table or line[1] in self.__ioi_table:
                        if line[0] in self.__rri_table:
                            add=self.__rri_table[line[0]]
                        elif line[1] in self.__rri_table:
                            add=self.__rri_table[line[1]]
                        elif line[0] in self.__ioi_table:
                            add=self.__ioi_table[line[0]]
                        elif line[1] in self.__ioi_table:
                            add=self.__ioi_table[line[1]]
                        key=self.__format2bin(LC, 'dec' , 12)
                        value=add
                        self.__bin[key]=value
                        LC+=1
